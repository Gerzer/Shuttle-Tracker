...
actor RefreshSequence: AsyncSequence, AsyncIteratorProtocol {
	...
	func next() async -> RefreshType? {
		do {
			...
			#log(system: Logging.system, level: .error, doUpload: true, "Refresh sequence canceled: \(error, privacy: .public)")
			return nil
		}
		do {
			...
			#log(system: Logging.system, level: .error, doUpload: true, "Refresh sequence production task failed: \(error, privacy: .public)")
			return nil
		}
	}
